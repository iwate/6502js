...
function SimulatorWidget(node) {
  ...
  function initialize() {
    ...
    $node.find('.databody').keypress(ui.initialize);
  ...
  function embedData() {
    var text = $node.find('.databody').val();
    var binary = text.split(' ').map(hex => parseInt(hex, 16));
    var head = parseInt($node.find('.datahead').val(), 16);
    $.each(binary, function(i, byte) {
      memory.storeByte(head + i, byte);
    });
  }
